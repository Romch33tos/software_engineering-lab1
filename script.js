(function () {
  ...
  function createPlanRow(role, days, loadPercent) {
    ...
    row.innerHTML =
      '<td><input value="Этап" style="width:100%"></td>' +
  ...
  function updateResume(ak, kpData, annualEffect, paybackPeriod, efficiencyRatio) {
    var html = '<div>';
    html += 'Ak: ' + ak.toFixed(3) + ' | ';
    html += 'Kп проект: ' + kpData.kpProject.toFixed(0) + ' руб. | ';
    html += 'Аналог: ' + kpData.kpAnalog.toFixed(0) + ' руб.';
    html += '</div><div>';
    html += 'Годовой эффект: ' + annualEffect.toFixed(0) + ' руб./год | ';
    html += 'Срок окупаемости: ' + (isFinite(paybackPeriod) ? paybackPeriod.toFixed(2) + ' лет' : '> 10 лет') + ' | ';
    html += 'Eф: ' + efficiencyRatio.toFixed(3);
    html += '</div>';
